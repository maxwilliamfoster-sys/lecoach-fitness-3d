...
m_floor=M('Floor',(0.11,0.11,0.12,1),0.9); m_wall=M('WallExt',(0.10,0.10,0.11,1),0.9)
m_part=M('Partition',(0.14,0.14,0.15,1),0.9); m_blue=M('Blue',(0.03,0.11,0.55,1),0.5)
...
HGT=4.0; TE=0.2; H=3.0; TP=0.15   # wall height, ext thickness, interior height/thickness
...
def poly(name,verts,m):
    ...
    bm=bmesh.new(); vs=[bm.verts.new((v[0],v[1],v[2])) for v in verts]; bm.faces.new(vs); bm.to_mesh(me); bm.free()
...
def label(text,x,y,size=0.9):
    ...
    o.name='LBL_'+text.split()[0]; o.data.body=text; o.data.size=size
    o.data.align_x='CENTER'; o.data.align_y='CENTER'; o.data.materials.append(m_txt)
    for c in o.users_collection: c.objects.unlink(o)
    col.objects.link(o); return o

# ---------------- STEPPED footprint ----------------
# front(entrance)=-6.5 ; back protrudes on left(leg) & right(functional), recessed in centre(main)
FY = -6.5      # front line
outline = [(-16,FY),(16,FY),(16,8),(10,8),(10,4.5),(-5,4.5),(-5,8),(-16,8)]
# floor + ceiling as the stepped polygon
poly('Floor',   [(x,y,0.0)  for x,y in outline], m_floor)
poly('Ceiling', [(x,y,HGT)  for x,y in outline][::-1], m_ceil)

# exterior walls following the outline (each edge)
def extwall(name,x0,y0,x1,y1):
    if abs(y1-y0)<1e-6:  # horizontal (along X)
        box(name, abs(x1-x0)+TE, TE, HGT, (x0+x1)/2, y0, HGT/2, m_wall)
    else:                # vertical (along Y)
        box(name, TE, abs(y1-y0)+TE, HGT, x0, (y0+y1)/2, HGT/2, m_wall)
edges=[('E_front',-16,FY,16,FY),('E_right',16,FY,16,8),('E_funcback',16,8,10,8),
       ('E_funcstep',10,8,10,4.5),('E_mainback',10,4.5,-5,4.5),('E_legstep',-5,4.5,-5,8),
       ('E_legback',-5,8,-16,8),('E_left',-16,8,-16,FY)]
for e in edges: extwall(*e)
# a few tie beams across the main body for industrial character
for x in (-8,-2,4): box(f'Tie_{x}',0.14,11,0.2,x,-1,HGT-0.25,m_steel)

# ---------------- rooms ----------------
# LEG ROOM back-left, deep (fills leg bump) x[-16,-5] y[0.5,8]
wallX('P_leg_S',-16,-5,0.5,m_part,gaps=[(-10.0,2.6)])
wallY('P_leg_E',0.5,8,-5,m_part,gaps=[(4.0,1.2)])
box('LegPlatform',9.0,5.0,0.06,-11,3.8,0.03,m_wood)
# TOILETS/SHOWERS front-left x[-16,-10.5] y[-6.5,-0.5]
...
# RECEPTION front-centre x[-2.5,2.5] y[-6.5,-3.5]
wallY('P_recep_W',-6.5,-3.5,-2.5,m_part); wallY('P_recep_E',-6.5,-3.5,2.5,m_part)
wallX('P_recep_N',-2.5,2.5,-3.5,m_part,gaps=[(0,1.2)])
box('ReceptionDesk',2.6,0.7,1.0,0.0,-4.6,0.5,m_wood)
box('EntranceMark',1.2,0.05,HGT,0.0,FY+0.06,HGT/2,m_blue)
# OFFICES (two) x[3.5,7.5] y[-6.5,-2.5]
wallY('P_off_W',-6.5,-2.5,3.5,m_part); wallY('P_off_E',-6.5,-2.5,7.5,m_part)
wallX('P_off_N',3.5,7.5,-2.5,m_part,gaps=[(5.5,1.0)])
wallX('P_off_div',3.5,7.5,-4.5,m_part,gaps=[(5.5,0.9)])
# ARM x[7.5,9.5] y[-6.5,-3]
wallX('P_arm_N',7.5,9.5,-3.0,m_part,gaps=[(8.5,0.9)])
# FUNCTIONAL big right wing x[9.5,16] y[-6.5,8]
wallY('P_func_W',-6.5,8,9.5,m_part,gaps=[(-4.5,0.9),(2.5,1.6)])
box('FuncFloor',6.5,14.5,0.04,12.75,0.75,0.02,m_floor)
# blue accents
box('Accent_recepN',5,0.06,0.25,0,-3.5,0.13,m_blue)
box('Accent_funcW',0.06,14.5,0.25,9.5,0.75,0.13,m_blue)

# ---------------- labels ----------------
label('LEG ROOM',-11,4.0,0.95); label('MAIN ROOM',2.0,1.5,1.1)
label('TOILETS / SHOWERS',-13.2,-3.4,0.55); label('RECEPTION',0,-5.2,0.45)
label('OFFICE',5.5,-3.5,0.4); label('OFFICE',5.5,-5.4,0.4)
label('ARM',8.5,-4.8,0.4); label('FUNCTIONAL ROOM',12.75,3.0,0.85)

# ---------------- top plan render ----------------
hide=['Ceiling','Tie_-8','Tie_-2','Tie_4']
for n in hide:
    o=col.objects.get(n)
    if o: o.hide_render=True
cam_data=bpy.data.cameras.new('PlanCam'); cam_data.type='ORTHO'; cam_data.ortho_scale=34
cam=bpy.data.objects.new('PlanCam',cam_data); scene.collection.objects.link(cam)
cam.location=(0,0.75,40); cam.rotation_euler=(0,0,0); scene.camera=cam
